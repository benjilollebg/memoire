...
static uint32_t nb_core = 7; /* The number of Core working on the NIC (max 7) */
...
dma_read(struct doca_pci_bdf *pcie_addr, char *rings[], size_t size) {
    ...
    for (index = 0; index < nb_core; index++) {
        ...
        result = doca_mmap_set_permissions(state[index].src_mmap, DOCA_ACCESS_DPU_READ_WRITE);
        if (result != DOCA_SUCCESS) {
            host_destroy_core_objects(&state[index]);
            return result;
        }
        /* Populate the memory map with the allocated memory */
        result = doca_mmap_set_memrange(state[index].src_mmap, rings[index], size);
        if (result != DOCA_SUCCESS) {
            host_destroy_core_objects(&state[index]);
            return result;
        }

        result = doca_mmap_start(state[index].src_mmap);
        if (result != DOCA_SUCCESS) {
            host_destroy_core_objects(&state[index]);
            return result;
        }
        /* DOCA : Export DOCA mmap to enable DMA on Host*/
        result = doca_mmap_export_dpu(state[index].src_mmap, state[index].dev, (const void **)&export_desc, &export_desc_len);
        if (result != DOCA_SUCCESS) {
            host_destroy_core_objects(&state[index]);
            return result;
        }
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < nb_core; i++) {
        rings[i] = (char *)aligned_alloc(0x1000,size);
        if (rings[i] == NULL) {
